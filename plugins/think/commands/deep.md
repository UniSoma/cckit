---
description: Multi-model structured thinking — analyze a topic through 3-5 mental model lenses with cross-model tension synthesis
argument-hint: <topic or question with genuine tradeoffs>
...
<framework_loading>
For each selected model, load its framework definition from the command file:

1. Use Read to load the command file at the path relative to this plugin's commands directory. The command files are sibling files in the same `commands/` directory as this file. For a model with slug `{model-slug}`, read `commands/{model-slug}.md` (relative to the plugin root).

   Since you are running as the orchestrator, use the path pattern:
   - Find the plugin root by noting that this command is at `plugins/think/commands/deep.md`
   - The sibling command files are at `plugins/think/commands/{model-slug}.md`
   - Use the working directory to construct the full path: `$(pwd)/plugins/think/commands/{model-slug}.md`
   - If the file is not found at that path, try using Glob: `**/plugins/think/commands/{model-slug}.md`

2. Extract the content AFTER the YAML frontmatter (after the closing `---` marker). This gives you the framework body containing objective, process, output_format, and success_criteria sections.

3. Store this as the FRAMEWORK parameter to pass to the lens agent.

**IMPORTANT**: Read ALL command files before spawning any agents. Framework loading must complete before parallel spawning begins.
</framework_loading>
...
Task tool:
  ...
  prompt: |
    ...
    FRAMEWORK:
    {framework content extracted from command file}
